...
(function(w) {
    ...
    // 正しいブロックのDOM要素を取得
    let correctBlockId = p.id;
    let correctBlockElement = document.getElementById(correctBlockId);

    // 擬似的なイベントを作成し、正しいブロックをターゲットにする
    let fakeEvent = {
        clientX: (p.cell * blockSize + (p.cell + 1) * blockSize) / 2 + body.offsetLeft, // 正しいブロックの中心X座標を計算
        clientY: (touchArea[0] + touchArea[1]) / 2, // タッチ可能エリアの中央Y座標
        target: correctBlockElement, // 正しいブロックの要素をターゲットに設定
        // モバイル環境を考慮して targetTouches も設定（必要であれば）
        targetTouches: [{ clientX: (p.cell * blockSize + (p.cell + 1) * blockSize) / 2 + body.offsetLeft, clientY: (touchArea[0] + touchArea[1]) / 2 }]
    };

    gameTapEvent(fakeEvent); // 正しいブロックを「クリック」
}
